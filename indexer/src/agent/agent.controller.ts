...
export class AgentController {
  ...
  async getAgent(@Param('agentId') agentId: string) {
    ...
    try {
      const account = await (this.program.account as any).registryAccount.fetch(
        registryPda,
      );
      ...
      return {
        ...
        pendingAttestationCount: account.pendingAttestationCount,
        challengedAttestationCount: account.challengedAttestationCount,
