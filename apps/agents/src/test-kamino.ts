...
async function buildKaminoTx(
  ...
  const reserveList = Array.isArray(reserves)
    ? reserves
    : (reserves?.reserves ?? []);
  const reserve = reserveList.find(
    (r: any) => r.liquidityTokenMint === tokenMint,
  );
  ...
  if (amount === "max") {
    ...
    const ob = Array.isArray(obligations)
      ? obligations[0]
      : obligations?.obligations?.[0];
    const borrow = ob?.state?.borrows?.find(
      (b: any) => b.borrowReserve === reserve.reserve,
    );
...
async function showPosition(wallet: string) {
  ...
  try {
    ...
    const solPrice = prices?.prices?.find(
    ...
    reserveList.slice(0, 6).forEach((r: any) => {
      ...
      console.log(
        `  ${sym.padEnd(8)} | Supply ${(supply * 100).toFixed(2)}% | Borrow ${(borrow * 100).toFixed(2)}%`,
      );
    });

    // Get user position
    const obligations = await getUserObligations(wallet);
    const obList = Array.isArray(obligations)
      ? obligations
    ...
    const ob = obList[0];
    const deposits =
      ob.state?.deposits?.filter(
        (d: any) => d.depositReserve !== "11111111111111111111111111111111",
      ) ?? [];
    const borrows =
      ob.state?.borrows?.filter(
        (b: any) => b.borrowReserve !== "11111111111111111111111111111111",
      ) ?? [];
    console.log("\n=== Your Position ===");
    console.log(`Deposits: ${deposits.length}`);
    deposits.forEach((d: any) =>
      console.log(
        `  Reserve: ${d.depositReserve.slice(0, 8)}... Amount: ${d.depositedAmount}`,
      ),
    );
    ...
    borrows.forEach((b: any) =>
      console.log(
        `  Reserve: ${b.borrowReserve.slice(0, 8)}... Amount: ${b.borrowedAmountSf}`,
      ),
    );

    console.log(
      `Deposited : $${parseFloat(ob.depositedValue ?? ob.totalDeposit ?? "0").toFixed(2)}`,
    );
    console.log(
      `Borrowed  : $${parseFloat(ob.borrowedValue ?? ob.totalBorrow ?? "0").toFixed(2)}`,
    );
    ...
    if (ob.deposits?.length > 0) {
      ...
      ob.deposits.forEach((d: any) =>
        console.log(
          `  ${d.symbol ?? d.tokenSymbol}: $${parseFloat(d.marketValueRefreshed ?? d.value ?? "0").toFixed(2)}`,
        ),
      );
    }
    if (ob.borrows?.length > 0) {
      ...
      ob.borrows.forEach((b: any) =>
        console.log(
          `  ${b.symbol ?? b.tokenSymbol}: $${parseFloat(b.marketValueRefreshed ?? b.value ?? "0").toFixed(2)}`,
        ),
      );
    }
  } catch (err: any) {
...
async function main() {
  ...
  console.log(
    `   ${decision.useMax ? "MAX" : decision.amount} ${decision.token}`,
  );
