...
async function showStatus(
  ...
  const { activeBin, userPositions } =
    await dlmm.getPositionsByUserAndLbPair(wallet);
...
async function addLiquidity(
  ...
  const dlmm = await DLMM.create(connection, new PublicKey(poolAddress));
  const { activeBin } = await dlmm.getPositionsByUserAndLbPair(
...
async function swapTokens(
  ...
  const inAmount = new BN(
    Math.floor(amountIn * Math.pow(10, tokenInfo.decimals)),
  );
  ...
  console.log(
    `Quote: ${amountIn} ${tokenIn} → out: ${quote.outAmount.toString()}`,
  );
...
async function main() {
  ...
  if (instruction === "help") {
    ...
    console.log(
      "  status                              — show position + active bin",
    );
    console.log(
      "  'add liquidity 0.01 SOL 1.5 USDC'  — add liquidity to SOL/USDC pool",
    );
    console.log(
      "  'remove all liquidity'              — remove 100% and close position",
    );
    console.log(
      "  'remove 50% liquidity'              — remove 50% of position",
    );
  ...
  if (decision.action === "status") {
    await showStatus(
      connection,
      agentKeypair.publicKey,
      decision.poolAddress ?? DEFAULT_POOL,
    );
    return;
  }
  ...
  try {
    switch (decision.action) {
      case "add_liquidity":
        console.log(
          `⚡ Adding liquidity: ${decision.amountX} SOL + ${decision.amountY} USDC`,
        );
        txSignatures = await addLiquidity(
          connection,
          agentKeypair,
          poolAddress,
          decision.amountX ?? 0.01,
          decision.amountY ?? 1.5,
          decision.binRange ?? 10,
        );
        break;
      ...
      case "swap":
        console.log(
          `⚡ Swapping ${decision.amountX} ${decision.tokenX} → ${decision.tokenY}`,
        );
        txSignatures = [
          await swapTokens(
            connection,
