...
import { executePumpFunTask, PUMP_PROGRAM_ID, PUMP_FEES_ID } from "./pumpfun/execute-pumpfun";
...
const GLOBAL_VOLUME_ACC = new PublicKey("Hq2wp8uJ9jCPsYgNHex8RtqdvMPfVGoYwjvF1ATiwn2Y");
...
function buildReceipt(p: { agentId: string; instruction: string; operation: string; mint: string; txSignature: string; keypair: Keypair }) {
    const receipt = { schema_version: "1.0", task_id: `pumpfun-${Math.floor(Date.now() / 1000)}`, agent_id: p.agentId, instruction: p.instruction, timestamp_unix: Math.floor(Date.now() / 1000), execution: { tx_signatures: [p.txSignature], programs_called: [PUMP_PROGRAM_ID], instructions_executed: [p.operation], token_mint: p.mint }, result: { status: "success", summary: `PumpFun ${p.operation} on ${p.mint.slice(0, 8)}...` } };
...
function getBondingCurvePDA(mint: PublicKey): PublicKey {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("bonding-curve"), mint.toBuffer()], PUMP_PROGRAM);
    return pda;
}
function getBondingCurveV2PDA(mint: PublicKey): PublicKey {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("bonding-curve-v2"), mint.toBuffer()], PUMP_PROGRAM);
    return pda;
}
function getGlobalPDA(): PublicKey {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("global")], PUMP_PROGRAM);
    return pda;
}
function getCreatorVaultPDA(creator: PublicKey): PublicKey {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("creator-vault"), creator.toBuffer()], PUMP_PROGRAM);
    return pda;
}
function getUserVolumeAccPDA(user: PublicKey): PublicKey {
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("user_volume_accumulator"), user.toBuffer()], PUMP_PROGRAM);
    return pda;
}
function getFeeConfigPDA(): PublicKey {
    const key = Buffer.from([1, 86, 224, 246, 147, 102, 90, 207, 68, 219, 21, 104, 191, 23, 91, 170, 81, 137, 203, 151, 245, 210, 255, 59, 101, 93, 43, 182, 253, 109, 24, 176]);
    const [pda] = PublicKey.findProgramAddressSync([Buffer.from("fee_config"), key], PUMP_FEES);
    return pda;
}
...
function getAssocBCurve(mint: PublicKey, tokenProgram: PublicKey): PublicKey {
    if (tokenProgram.equals(TOKEN_PROGRAM_ID)) {
        const [pda] = PublicKey.findProgramAddressSync([Buffer.from("associated-bonding-curve"), mint.toBuffer()], PUMP_PROGRAM);
        return pda;
    }
    return getAssociatedTokenAddressSync(mint, getBondingCurvePDA(mint), true, tokenProgram);
}

interface BCState { virtualTokenReserves: bigint; virtualSolReserves: bigint; realTokenReserves: bigint; realSolReserves: bigint; tokenTotalSupply: bigint; complete: boolean; creator: PublicKey; cashbackEnabled: boolean; }

function parseBC(data: Buffer): BCState {
    let o = 8;
    const vtr = data.readBigUInt64LE(o); o += 8;
    const vsr = data.readBigUInt64LE(o); o += 8;
    const rtr = data.readBigUInt64LE(o); o += 8;
    const rsr = data.readBigUInt64LE(o); o += 8;
    const tts = data.readBigUInt64LE(o); o += 8;
    const complete = data.readUInt8(o) === 1; o += 1;
    ...
    return { virtualTokenReserves: vtr, virtualSolReserves: vsr, realTokenReserves: rtr, realSolReserves: rsr, tokenTotalSupply: tts, complete, creator, cashbackEnabled };
}
...
function calcSellSol(s: BCState, tokensIn: bigint): bigint {
    const fee = 100n;
    const gross = (tokensIn * s.virtualSolReserves) / (s.virtualTokenReserves + tokensIn);
...
async function showStatus(connection: Connection, mintAddress: string) {
    ...
    if (!info) { console.log("Bonding curve not found."); return; }
    const s = parseBC(info.data);
    const price = s.virtualTokenReserves > 0n ? Number(s.virtualSolReserves) / Number(s.virtualTokenReserves) : 0;
    const mcap = s.virtualTokenReserves > 0n ? Number(s.virtualSolReserves) * 1e9 / Number(s.virtualTokenReserves) / 1e9 : 0;
    const prog = (1 - Number(s.realTokenReserves) / Number(s.tokenTotalSupply)) * 100;
    ...
    console.log(`Graduation     : ${prog.toFixed(1)}% (${s.complete ? "✅ COMPLETE" : "🔄 in progress"})`);
    console.log(`Real SOL       : ${(Number(s.realSolReserves) / 1e9).toFixed(4)} SOL`);
    console.log(`Tokens left    : ${(Number(s.realTokenReserves) / 1e6).toFixed(0)}`);
    console.log(`Creator        : ${s.creator.toString().slice(0, 8)}...`);
    console.log(`Cashback       : ${s.cashbackEnabled ? "✅ enabled" : "❌ disabled"}`);
    if (s.complete) console.log(`\n⚠️  Bonding curve complete — trade on PumpSwap instead`);
}

async function buyTokens(connection: Connection, kp: Keypair, mintAddress: string, solAmount: number, slippageBps: number): Promise<string> {
    const mint = new PublicKey(mintAddress);
    ...
    if (state.complete) throw new Error("Bonding curve complete — use PumpSwap instead");
    ...
    console.log(`  Token program  : ${tokenProgram.equals(TOKEN_PROGRAM_ID) ? "SPL" : "Token2022"}`);
    ...
    const userAta = getAssociatedTokenAddressSync(mint, kp.publicKey, false, tokenProgram);
    if (!await connection.getAccountInfo(userAta)) {
        tx.add(createAssociatedTokenAccountInstruction(kp.publicKey, userAta, kp.publicKey, mint, tokenProgram));
    }
...
async function sellTokens(connection: Connection, kp: Keypair, mintAddress: string, tokenAmount: number, slippageBps: number): Promise<string> {
    const mint = new PublicKey(mintAddress);
    const bcPDA = getBondingCurvePDA(mint);
    const bcV2 = getBondingCurveV2PDA(mint);
    const global = getGlobalPDA();
    ...
    if (state.cashbackEnabled) {
        keys.push({ pubkey: getUserVolumeAccPDA(kp.publicKey), isSigner: false, isWritable: true }); // 14
    }
...
async function main() {
    ...
    if (decision.action === "reject") { console.log(`⚠️  Rejected: ${decision.reason}`); return; }
    if (!decision.mint) { console.log("❌ No mint address provided"); return; }
    if (decision.action === "status") { await showStatus(connection, decision.mint); return; }
    ...
    try {
        if (decision.action === "buy") {
            ...
            txSignature = await buyTokens(connection, kp, decision.mint, decision.solAmount ?? 0.001, decision.slippageBps ?? 500);
        } else {
            console.log(`⚡ Selling on PumpFun bonding curve`);
            console.log(`  Mint   : ${decision.mint.slice(0, 8)}...`);
            console.log(`  Tokens : ${decision.tokenAmount}`);
            txSignature = await sellTokens(connection, kp, decision.mint, decision.tokenAmount ?? 0, decision.slippageBps ?? 500);
        }
    } catch (err: any) {
    ...
    const { outputHash, agentSignature } = buildReceipt({ agentId: kp.publicKey.toBase58(), instruction, operation: decision.action, mint: decision.mint, txSignature, keypair: kp });
