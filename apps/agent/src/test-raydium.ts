...
function buildReceipt(params: {
    ...
    txSignatures: string[]; programId: string; keypair: Keypair;
}) {
    const receipt = {
        schema_version: "1.0",
        ...
        execution: {
            ...
            programs_called: [params.programId],
...
async function swapViaTradeApi(
    ...
    const amountInBase = Math.floor(amountIn * Math.pow(10, tokenInInfo.decimals)).toString();
    ...
    console.log(`Quote: ${amountIn} ${tokenIn} → ${outAmount.toFixed(6)} ${tokenOut}`);
...
async function addLiquidityCpmm(
    ...
    const raydium = await Raydium.load({
        connection, owner: agentKeypair, cluster: "mainnet", disableLoadToken: true,
    });
...
async function main() {
    ...
    if (instruction === "help") {
        ...
        console.log("  'add liquidity 1000000'          — add 0.001 SOL worth (CPMM)");
    ...
    if (decision.action === "status") {
        await showStatus(decision.poolId ?? DEFAULT_POOL, decision.poolType ?? DEFAULT_POOL_TYPE);
        return;
    }

    const poolId = decision.poolId ?? DEFAULT_POOL;
    const poolType = decision.poolType ?? DEFAULT_POOL_TYPE;
    const programId = poolType === "cpmm" ? RAYDIUM_CPMM
        : poolType === "clmm" ? RAYDIUM_CLMM
            : RAYDIUM_AMM;
    ...
    try {
        switch (decision.action) {
            case "swap":
                console.log(`⚡ Swapping ${decision.amountIn} ${decision.tokenIn} → ${decision.tokenOut}`);
