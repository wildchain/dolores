...
import { loadKeypairFromFile } from "@dolores/solana-utils";
...
async function fetchReputation(indexerUrl: string, agentId: string) {
  const res = await fetch(`${indexerUrl}/agents/${agentId}/reputation`);
...
export async function runCommand(opts: {
  ...
  try {
    ...
    console.log(
      `Submission tx   : ${result.submissionTx ?? "null (check indexer logs)"}`,
    );

    if (result.submissionTx) {
      console.log(
        `Explorer        : https://explorer.solana.com/tx/${result.submissionTx}?cluster=devnet`,
      );
    }
