...
export async function runCommand(opts: {
  ...
  const expectedHash = Buffer.from(
    await hashManifest(solTransferManifest),
  ).toString("hex");
  const agentHash = Buffer.from(await hashManifest(manifest)).toString("hex");

  if (agentHash !== expectedHash) {
    console.error(`❌ This agent is not authorized to perform SOL transfers.`);
    console.error(`   Agent template : ${manifest.template_id}`);
  ...
  console.log(`Template        : ${manifest.template_id} ✅`);
