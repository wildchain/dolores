...
const idlAdjudication = doloresAdjudicationIdl;
...
export async function challengeCommand(opts: {
  ...
  if (
    opts.failureType !== "missed-deadline" &&
    opts.failureType !== "out-of-scope-call"
  ) {
  ...
  try {
    ...
    console.log(
      `Explorer        : https://explorer.solana.com/tx/${tx3}?cluster=devnet\n`,
    );
  } catch (err: any) {
  ...
  console.log(
    `   dolores verify  --agent-id ${opts.agentId} --min-rep 0 --min-stake 0.1\n`,
  );
}
