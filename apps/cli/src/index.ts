...
program
  .command("assign")
  .description("Assign a task to an agent — registers instruction on-chain and notifies indexer")
  .requiredOption("--agent-id <pubkey>", "Agent public key to assign task to")
  .requiredOption("--instruction <text>", "Natural language instruction for the agent (max 256 chars)")
...
program
  ...
  .requiredOption("--task-id <hex>", "Task ID (hex string from dolores assign output)")
  .option("--indexer <url>", "Indexer base URL", DEFAULT_INDEXER_URL)
  .action(async (opts) => {
