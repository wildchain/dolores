...
export class SyncService implements OnModuleInit, OnModuleDestroy {
  ...
  private async startEventListeners() {
    try {
      ...
      // BRIDGE: Listen for TaskCompleted emitted by complete_task() in
      // dolores_adjudication. This is how the agent runtime in apps/agent
      // signals task completion. The handler updates the task cache so
      // the frontend reflects the new state in real time.
      const taskCompletedListenerId = adjudicationProgram.addEventListener(
        'TaskCompleted',
        async (event: any) => {
          await this.handleTaskCompleted(event);
        },
      );
      this.listenerIds.push(taskCompletedListenerId);
  ...
  /**
   * BRIDGE: Handle TaskCompleted event emitted by complete_task() in
   * dolores_adjudication. This is the on-chain signal from the agent
   * runtime (apps/agent) that a task has finished. We update the cache
   * status to 'completed' and store the output_hash so the frontend
   * can show the receipt.
   *
   * Note: this does NOT call submit_attestation on-chain. That happens
   * via the indexer's /receipts/upload flow, which the agent calls
   * separately. This handler only mirrors on-chain state into the cache.
   */
  private async handleTaskCompleted(event: any) {
    const taskId = Buffer.from(event.taskId).toString('hex');
    const agentId = event.agent.toBase58();
    const outputHashHex = Buffer.from(event.outputHash).toString('hex');
    const completedAt =
      typeof event.completedAt?.toNumber === 'function'
        ? event.completedAt.toNumber()
        : Number(event.completedAt);

    this.logger.log(
      `TaskCompleted: Agent ${agentId}, Task ${taskId.slice(0, 16)}…, outputHash ${outputHashHex.slice(0, 16)}…`,
    );
    ...
    try {
      await this.tasksService.updateTaskStatus(
        ...
        {
          completedAt,
          // Store output_hash in receiptUrl field for now (no Arweave yet).
          // Frontend can use this to verify against on-chain TaskRecord PDA.
          receiptUrl: `onchain:${outputHashHex}`,
        },
      );
      this.logger.log(`Task ${taskId.slice(0, 16)}… marked completed in cache`);
    } catch (error) {
      this.logger.error(
        `Failed to update task cache for ${taskId}`,
        error,
      );
    }
  }
