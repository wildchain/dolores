...
export async function verifyCommand(opts: {
  ...
  // 3. Print result
  console.log(`─────────────────────────────────────────`);
  console.log(`Reputation      : ${reputation} / 10000  ${meetsReputation ? "✅" : "❌"}  (min: ${opts.minReputation})`);
  console.log(`Declared stake  : ${(declaredStake / LAMPORTS_PER_SOL).toFixed(4)} SOL  ${meetsStake ? "✅" : "❌"}  (min: ${opts.minStakeSol} SOL)`);
  console.log(`Slash count     : ${slashCount}  ${notBanned ? "✅" : "❌"}  (max: 2)`);
  console.log(`─────────────────────────────────────────`);
  ...
  if (trusted) {
    ...
    if (!meetsReputation) {
      ...
      console.log(`   Reputation too low — needs ${needed} more points (~${tasksNeeded} more tasks)`);
    }
    if (!meetsStake) {
      const neededSol = ((minStakeLamports - declaredStake) / LAMPORTS_PER_SOL).toFixed(4);
      console.log(`   Stake too low — stake ${neededSol} more SOL`);
      console.log(`   Run: dolores stake --agent-id ${opts.agentId} --amount ${neededSol}`);
    }
    if (!notBanned) {
